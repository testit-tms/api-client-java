...
public class ProjectExportQueryModel {
  ...
  * @throws IOException if the JSON Element is invalid with respect to ProjectExportQueryModel
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!ProjectExportQueryModel.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!ProjectExportQueryModel.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ProjectExportQueryModel` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      // ensure the optional json data is an array if present
      if (jsonObj.get("workItemIds") != null && !jsonObj.get("workItemIds").isJsonNull() && !jsonObj.get("workItemIds").isJsonArray()) {
