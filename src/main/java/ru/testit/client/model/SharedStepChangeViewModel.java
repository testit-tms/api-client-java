...
public class SharedStepChangeViewModel {
   ...
   * @throws IOException if the JSON Element is invalid with respect to SharedStepChangeViewModel
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!SharedStepChangeViewModel.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!SharedStepChangeViewModel.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `SharedStepChangeViewModel` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      for (int i = 0; i < jsonArraysteps.size(); i++) {
        WorkItemStepChangeViewModel.validateJsonElement(jsonArraysteps.get(i));
      };
  }
